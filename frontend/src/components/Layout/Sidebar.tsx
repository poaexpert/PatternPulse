...
export default function Sidebar() {
  ...
  // 5-tap logo easter egg for admin access
  ...
  const navItems: NavItem[] = [
    ...
    { id: 'calendar', label: 'Eco Calendar', icon: (
      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
    ...
    { id: 'pattern-scanner', label: 'Pat. Scanner', icon: <CameraIcon /> },
    { id: 'risk-calc',       label: 'Risk Calc',    icon: <CalculatorIcon /> },
    ...
    { id: 'multi-tf',        label: 'Multi-TF',     icon: <GridIcon /> },
    { id: 'paper-trade',     label: 'Paper Trade',  icon: <DollarIcon /> },
  ...
  return (
    <aside className="w-16 lg:w-56 bg-terminal-card border-r border-terminal-border flex flex-col shrink-0 z-20">
      {/* Logo — tap 5× to open admin login */}
      <div className="px-3 lg:px-4 py-4 border-b border-terminal-border">
        <div className="flex items-center gap-2.5">
          <div onClick={handleLogoTap} className="w-8 h-8 bg-terminal-cyan/10 rounded-lg flex items-center justify-center shrink-0 border border-terminal-cyan/20 cursor-pointer select-none">
      ...
      {/* Admin link — only visible when already logged in as admin */}
      {isAdminLoggedIn && (
